...
export async function createLesson(input: CreateLessonInput) {
  ...
  const existingLesson = await prisma.classLesson.findFirst({
    where: {
  ...
  if (existingLesson) {
    throw new Error(
      "A lesson with the same class, content, and date already exists"
    );
  }
...
export async function getLessonById(params: GetLessonParams) {
...
export async function getClassLessons(params: GetClassLessonsParams) {
  // Verify class exists
...
export async function updateLesson(
  ...
  if (input.lessonDate) {
    const conflictingLesson = await prisma.classLesson.findFirst({
      where: {
        id: { not: params.id },
  ...
  if (input.contentId) updateData.contentId = input.contentId;
...
export async function deleteLesson(params: GetLessonParams) {
  // Verify lesson exists
  const existingLesson = await prisma.classLesson.findUnique({
    where: {
      id: params.id,
